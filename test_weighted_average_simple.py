...
def calc_avg_weighted_score(votes, weights):
    """
    Calculate average weighted score.
    Each vote is a dict with: confidence, quality
    """
    if not votes:
        return 0.0
    total_weighted = sum(v['confidence'] * v['quality'] * weights.get(v.get('agent', 'default'), 1.0) for v in votes)
    total_weight = sum(weights.get(v.get('agent', 'default'), 1.0) for v in votes)
...
def test_three_weak_votes():
    ...
    votes = [
        {'agent': 'Agent1', 'confidence': 0.35, 'quality': 0.35},
        {'agent': 'Agent2', 'confidence': 0.35, 'quality': 0.35},
        {'agent': 'Agent3', 'confidence': 0.35, 'quality': 0.35},
    ]

    weights = {'Agent1': 1.0, 'Agent2': 1.0, 'Agent3': 1.0}

    avg_score = calc_avg_weighted_score(votes, weights)

    print(f"Votes: {len(votes)} × 0.35 confidence")
    print(f"Average score: {avg_score:.3f}")

    # Should be confidence * quality = 0.35 * 0.35 = 0.1225
    expected = 0.35 * 0.35
...
def test_mixed_confidence_votes():
    ...
    votes = [
        {'agent': 'Agent1', 'confidence': 0.80, 'quality': 0.80},
        {'agent': 'Agent2', 'confidence': 0.30, 'quality': 0.30},
        {'agent': 'Agent3', 'confidence': 0.30, 'quality': 0.30},
    ]
    ...
    # Manual calculation: (0.64 + 0.09 + 0.09) / 3 = 0.2733
...
def test_different_weights():
    ...
    votes = [
        {'agent': 'Agent1', 'confidence': 0.60, 'quality': 0.60},
        {'agent': 'Agent2', 'confidence': 0.40, 'quality': 0.40},
        {'agent': 'Agent3', 'confidence': 0.50, 'quality': 0.50},
    ]
    ...
    # Manual calculation: (0.54 + 0.16 + 0.125) / 3.0 = 0.275
...
def test_single_vote():
    ...
    votes = [
        {'agent': 'Agent1', 'confidence': 0.70, 'quality': 0.65},
    ]
